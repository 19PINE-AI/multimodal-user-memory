...
\paragraph{Design choices for a faithful read.} Four details determine whether the read recovers the encoder's nearest neighbour cleanly, and getting them right is what lets the mechanism work with no training. (i) \emph{Do not} divide the attention logits by $\sqrt{D}$: with normalized keys, that flattens every cosine difference into near-uniform attention. (ii) Set the sharpness $\beta$ high, so the attention is a near-hard argmax over the bank rather than a diffuse average. (iii) Attach the hook at the output head, not at an intermediate layer, so the residual reaches the logits undiluted. (iv) Set the gain $g$ large, so the retrieved marker dominates the hidden state and the read is exact; on models with untied output embeddings, whose output-embedding norms run smaller, $g$ must be larger still (Appendix~\ref{app:tables}, Table~\ref{tab:gainrobust}). With these four choices the read reproduces the encoder's recall exactly (\S\ref{sub:core}), and registering a user is a single row append with no training at all.
...
The sources are all standard: LFW~\citep{lfw} and AgeDB~\citep{agedb} read with ArcFace~\citep{arcface}, WikiArt~\citep{wikiart} with mid-layer CLIP~\citep{clip}, LibriSpeech~\citep{librispeech} with ECAPA-TDNN~\citep{ecapa}, ESC-50~\citep{esc50} with AST~\citep{ast}, and RAVDESS~\citep{ravdess} with a wav2vec2 emotion encoder~\citep{wav2vec2}. The benchmark deliberately tests \emph{memory}, not perception: every sub-modality has a fixed, off-the-shelf encoder that any method may use as black-box infrastructure, so no one wins by having a better encoder. The interface is just \texttt{register} and \texttt{recall}. Data is cross-session: a registered sample and its query come from different recordings. The identities used to train the memory never overlap with those it is evaluated on. For a memory of $N$ identities we register one sample each, then issue cross-condition queries and ask the method to name the right one. Throughout the paper, \emph{recall} means recall@1: the fraction of queries for which the top-scoring registered identity is the correct one --- for retrieval, the encoder's nearest neighbour; for the in-model read, the argmax over the registered marker logits. For tone of voice, each registered ``identity'' is a (speaker, emotional-state) pair, so recalling it means matching a paralinguistic state across separate utterances, something a per-utterance caption cannot do without the user's own history.

For statistical rigor we expand these five conceptual sub-modalities into a $12$-domain, $1{,}080$-task benchmark across multiple datasets and encoders --- each domain a (dataset, encoder) pair, enumerated with its pool size and per-domain recall in Appendix Table~\ref{tab:domains} (Section~\ref{sec:results}, Figure~\ref{fig:crossdomain}) --- and report a text-only baseline and a chance floor on every comparison. Throughout, our reference point is \textbf{embedding retrieval}: the same encoder, cosine nearest-neighbour over the registered keys. We call it the \emph{encoder ceiling}, because it is the best one can do with the encoder's own similarity, and our in-model read reproduces it rather than exceeding it.
...
The baseline a system would otherwise use is a text note: describe the perception in words, store the sentence, retrieve by text similarity. We test this on \emph{all five modalities} and give it its best shot --- an LLM writes a re-identification note (Qwen2.5-VL~\citep{qwen25vl} for images, Qwen2.5-Omni~\citep{qwen25omni} for audio), a sentence encoder embeds it, and recognition is cosine nearest-neighbour over the notes --- scored against the encoder on identical draws. The result is a clean router gradient (Figure~\ref{fig:textablation}): the note's recall slides from nearly the encoder's where the signal is \emph{nameable} (an acoustic scene, ``someone typing on a keyboard'') down toward chance where it is a perceptual \emph{identity} (a voice, whose ``high-pitched, with an accent'' fits countless speakers). Text is a competent memory for what can be said and a failing one for what cannot --- precisely the line the router draws between the text store and the parametric bank (\S\ref{sec:discussion}).
...
Once the referent is localized and encoded, recognition is the bank's job, and the bank reproduces the encoder. With a sharp read the attention approximates a hard argmax over the same encoder cosine that embedding retrieval uses, so its recall matches the encoder's, neither better nor worse. A paired protocol that scores both on identical registrations and queries with the target slot randomised confirms this at every bank size from $N{=}5$ to $N{=}1000$ ($|\Delta|\le 0.001$ at each $N$, $20$ draws; Table~\ref{tab:headline}, Figure~\ref{fig:scaling}): the memory ties retrieval on random banks and on adversarial banks of near-identical look-alikes, and a learned metric on the same features lands in the same place (Appendix~\ref{app:tables}). The tie depends on the read being sharp: run with an under-sharpened read, the same sweep trails the cosine at $N\ge300$ (Appendix~\ref{app:tables}), a gap the constants of Section~\ref{sec:method} eliminate.
...
The natural baseline is retrieve-and-reprompt: cosine-retrieve the nearest registered face, recover its name, and re-prompt the model. Because both routes recognise with the same encoder cosine and reason with the same frozen model, they reach \emph{identical accuracy} --- on composition and on a multi-perception ``how many distinct people are here?'' task alike (Table~\ref{tab:pipeline}). On cost the pipeline is in fact \emph{cheaper} for pure recognition, since counting distinct identities needs only cosine and no model forward at all. Against this strong baseline the parametric memory is not a faster or more accurate recogniser, and we do not claim it is. Its advantage is architectural, and it is worth stating exactly when that matters. The pipeline is a two-pass design: an orchestrator must decide \emph{before} generation that a recognition will be needed, query the index, and re-prompt with the answer. The in-model read makes no such decision --- the bank is consulted at every perceptual position of the generation the agent is already running, so recognition composes with whatever the model is midway through producing (\S\ref{sub:compose}), interleaves across many perceptions in one stream, and ships as a single frozen-model artifact with no external index to host, synchronize, or secure. Where a deployment can always afford the second pass, the pipeline is a fine choice; the parametric memory is for the agent that cannot know in advance when a perception will need a name.
...
The perceptual memory is meant to sit beside an ordinary text memory, not perturb it. The hook adds a residual only at perceptual positions, so with the bank installed and populated the model's text-only behaviour is preserved: the top-1 next-token prediction matches the untouched model on every probe, the populated bank produces exactly the same logits as an empty one, and with the hook idle the logits are bitwise identical (max $|\Delta\text{logit}|{=}0$; the residual $0.375$ deviation on the wrapped forward comes from the host's \texttt{inputs\_embeds} numerics and is present with the bank empty or full alike). Register faces and voices together and the two banks stay independent without any training (cross-modal leakage $0.017$ and $0.067$). An agent can keep ``my favourite restaurant is in Paris'' in its text store and ``how Bibi looks across lighting and age'' in its perceptual bank, and lose nothing on either. The discrete-codebook design we tried before this one, which snapped each perception to one of $K$ learned codes, never escaped $\sim$0.07 recall past a few hundred identities for the same reason captioning fails --- a categorical bottleneck discards the signal the encoder preserved; we keep it as a contrast in Appendix~\ref{app:patha}.

\subsection{An end-to-end multi-session agent}
\label{sub:agent}

The pieces compose into a working agent. We build one that runs the full system --- a frozen language model with the \attmem{} perceptual bank and a text store of facts --- and enroll a growing population of users across sessions, each with a face (registered from one photo) and a bound fact. A returning user is perceived cross-condition (a different photo), and the agent must identify them, answer a question about them, and reject anyone it has never met. We evaluate as the enrolled population grows from ten to eighty users, over five seeds (Figure~\ref{fig:agent}). The perceptual capabilities scale gracefully: identification holds at $0.86$ and stranger rejection at $0.81$ at eighty users, tracking the encoder as everywhere (the $0.96$--$0.99$ rejection of \S\ref{sub:core} is measured at twenty enrolled; the operating point tightens as enrollment grows, exactly as the encoder's separability does). Fact recall exposes the router directly. Routed through the text store --- the perception recalls the name, the name keys an exact fact lookup --- accuracy tracks identification ($0.86$ at eighty users). Attempted instead \emph{in-model}, by binding all facts in the prompt and reading the answer in one pass, it collapses beyond a few dozen users ($0.55$ at twenty-five, $0.04$ at fifty), because a $3$B model cannot hold fifty name--fact bindings in context. This is the router thesis made concrete: perceptual identity belongs in the parametric bank, where it scales, and exact facts belong in a text store, where lookup stays exact. The production path --- perceptual bank for \emph{who}, text store for \emph{what} --- sustains end-to-end task success at $0.83$ for eighty users inside a single frozen model, with no external index.
...
\textbf{Text-first agent memory.} The conversational-memory line --- Mem0~\citep{mem0}, MemGPT and its Letta successor~\citep{memgpt,letta}, MemoryLLM~\citep{memllm}, M3-Agent~\citep{m3agent}, benchmarked by LongMemEval~\citep{longmemeval} --- stores transcripts and captions in a text index and retrieves by similarity. These systems define the captionable half of user memory that ours composes with; none keeps a perception in its native modality, which is the half this paper adds (\S\ref{sub:textcant} measures what that omission costs).

\textbf{Personalized vision-language models.} MyVLM~\citep{myvlm}, Yo'LLaVA~\citep{yollava}, MC-LLaVA~\citep{mcllava}, RAP~\citep{rap}, Online-PVLM~\citep{onlinepvlm}, and TAME~\citep{tame} personalize a VLM to user-specific concepts, most by learning an artifact per concept --- a classifier head, a soft token, a projection. Our setting differs in cost profile and mechanism: registration is a training-free $\mathcal{O}(1)$ row append, and Appendix Table~\ref{tab:perconcept} finds that the learned per-concept artifacts do not buy accuracy over the encoder's own cosine on cross-condition identity, while costing per-identity gradient descent.

\textbf{Retrieval inside the model.} \attmem{} borrows the shape of attention over an external bank from $k$NN-LM~\citep{khandelwal2020nearest}, Memorizing Transformers~\citep{wu2022memorizing}, and RETRO~\citep{retro}, which retrieve from token or chunk stores to improve language modelling. We retarget the mechanism: the keys are perceptual-encoder identities rather than hidden states of past text, the bank holds a user's persistent percepts rather than a corpus, and the read lands at the output head of a frozen model with no training.

\textbf{Perception-to-token bridges and speaker recognition.} Flamingo's resampler~\citep{flamingo} and BLIP-2's Q-Former~\citep{blip2} learn to project perception into a language model's token space; our read instead injects a single marker whose value is the model's \emph{own} embedding, which is what makes it training-free. The speaker-recognition and diarization literature~\citep{voxceleb,ecapa,diarizationreview} solves who-spoke-when as a standalone pipeline returning labels; we consume the same encoders as grounded identity keys the model conditions on, rather than as an external verdict.
...
\textbf{Privacy and responsible use.} A perceptual user memory stores biometric templates --- faceprints and voiceprints --- and must be held to the standards that govern them. Enrollment should be consented and explicit (the ``remember her'' act of Figure~\ref{fig:worked} is an affirmative request, not passive collection), the bank kept on-device or encrypted at rest, and the same mechanism must not be repurposed for identifying people who never asked to be remembered. Two properties of the design help. Because one identity is one row, deletion is as cheap as registration: dropping the row removes the identity entirely, with no fine-tuned weights to unlearn --- a clean right-to-be-forgotten that per-concept tuning cannot offer. And because recall \emph{is} the encoder's, the memory inherits the encoder's documented demographic accuracy skews (face recognition most of all); deployments should evaluate the chosen encoder on their user population and prefer verification-style operating points (Figure~\ref{fig:openset}) with stranger rejection enabled, so an unenrolled person is refused rather than misassigned an enrolled name.

\textbf{Limitations.} (i) The memory cannot exceed the encoder. Its recall \emph{is} the encoder's --- paired to within $\pm0.001$ from $N{=}5$ to $N{=}1000$ (Table~\ref{tab:headline}, Figure~\ref{fig:scaling}) --- so at very large memories it declines exactly as the encoder's cosine does ($0.78$ at a thousand faces), and no in-model trick recovers more. (ii) Our largest cross-condition pool holds $2{,}180$ real identities, from which we measure recall to $N{=}1000$ registered; the constant-time behaviour is confirmed to ten thousand rows, but recall at that scale awaits data. (iii) The grounding step inherits the grounder's referring-expression resolution, and it is the component that degrades with complexity: on well-separated visual composites it is near-lossless to six referents (grounding $1.00$ through $K{=}5$), but a stress test that pastes jittered, rescaled faces onto photographic backgrounds drops grounding to $0.63$--$0.81$ and end-to-end recall well below its oracle (Appendix Table~\ref{tab:realistic}), and audio grounding of the fourth speaker falls to $0.69$; denser, occluded, or noisier scenes will stress it further, and detector and diarizer robustness --- not the encoder --- is the practical bottleneck there. (iv) Inside a VLM, native vision tokens are weak identity keys ($0.50$ vs.\ ArcFace's $0.95$ at $N{=}10$, Table~\ref{tab:vlm}), so an external, modality-specific encoder is required. (v) The one per-model constant (read-strength gain) is set by inspection; untied-embedding models need a larger value, which we have not automated. (vi) The closest prior systems (MyVLM~\citep{myvlm}, Online-PVLM~\citep{onlinepvlm}) have released neither code nor checkpoints; we compare against charitable reimplementations of their core mechanisms (Appendix~\ref{app:tables}, Table~\ref{tab:perconcept}).
