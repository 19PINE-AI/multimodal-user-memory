...
Our mechanism keeps this identity step and adds the two things it lacks: a VLM in front to resolve the referent, and a new home for the answer. Rather than return a neighbour from an external index, it stores the encoder key beside the model's own marker-token vector and reads the pair back by attention inside the model (Section~\ref{sec:method}), so the recalled identity is a token the model conditions on. Recognition is unchanged --- a sharp read is a hard argmax over the same encoder cosine --- so what the memory adds is not a higher score but grounding, an in-model home, $\mathcal{O}(1)$ registration, and portability to any frozen model.
...
Because the read reproduces the encoder's recall, the memory's quality \emph{is} the encoder's quality, and recall moves with encoder quality on the \emph{same} identities: on AgeDB, ArcFace reaches $0.95$ at ten identities and $0.75$ at a hundred, while a VLM's own vision tokens manage only $0.64$ and $0.34$ on those exact faces, and a learned metric on the same features does no better (Appendix~\ref{app:tables}). This is why the ``identify'' step uses a dedicated encoder rather than the model's native tokens, and it holds end-to-end inside a real VLM, where native-token recall is only $0.60$ against a face encoder's $0.93$ (Appendix Table~\ref{tab:vlm}). The store, in contrast, is model-agnostic: the read carries no encoder-specific machinery, so it ports to any frozen model without training (\S\ref{sub:core}). If a task needs better perceptual recall, then, the only remedy is a stronger encoder --- not a different language model or a cleverer read.
