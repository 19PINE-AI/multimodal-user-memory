...
We organize the evaluation around the three claims of the grounded memory. First, the half text drops is real and large: captions cannot carry perceptual identity (\S\ref{sub:textcant}). Second, grounding recovers it: VLM grounding plus an encoder identity reaches the correct-region oracle in cluttered scenes (\S\ref{sub:factored}). Third, the recognition core is the encoder, read in-model, which is a feature, not a shortfall (\S\ref{sub:core}). We then show that the in-model store enables composition with text memory (\S\ref{sub:compose}) at a cost no external index can match (\S\ref{sub:cost}) while preserving text behaviour exactly (\S\ref{sub:nonreg}).
