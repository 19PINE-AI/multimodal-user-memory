...
In deployment a perception rarely arrives pre-cropped: the referent sits among other people or objects, and the system must resolve the one the user means before encoding it. An ablation isolates the value of each step (Figure~\ref{fig:agentic}). A \emph{store-only} memory that embeds the whole scene cannot represent a chosen referent among distractors and recovers it at near chance --- below even a coarse text-only caption. \emph{Grounding} the referent first, then identifying it with the encoder, recovers the correct-region oracle, with the VLM resolving the right region on essentially every trial, on faces (ArcFace) and paintings (CLIP) alike. Retrieval coincides with the store throughout --- both are the encoder's nearest neighbour --- so the gain is grounding, not the matcher. Grounding is lossless and domain-general; what it recovers is bounded only by the encoder, modest for CLIP style and near-perfect for ArcFace identity. This is the decomposition made visible: the VLM resolves \emph{what} to remember and where it is, and a purpose-built encoder determines \emph{who} it is.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{figs/fig_agentic.pdf}
  \caption{Ablation of the grounded memory on cluttered scenes ($M{=}40$, two referents per scene). Capability increases left to right: a \emph{text-only} caption ($\dagger$, from the matched identity benchmark on clean crops, Figure~\ref{fig:textablation}); \emph{store-only}, embedding the whole scene with no grounding; \emph{grounded} (ground $+$ identify $+$ store, ours); and the correct-region \emph{oracle}. Store-only is near chance --- below even text-only --- while grounding recovers the oracle. Retrieval coincides with store throughout (both are the encoder's nearest neighbour), so the gain is grounding, not the matcher. Faces: ArcFace, landmark alignment, $5$ seeds; paintings: CLIP, no alignment, $3$ seeds; error bars $\pm$95\% CI.}
  \label{fig:agentic}
\end{figure}
