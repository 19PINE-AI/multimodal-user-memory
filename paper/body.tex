...
\begin{figure}[t]
  ...
  \caption{The grounded advantage is robust to scene clutter and grounder scale (faces, $M{=}40$). \textbf{Left:} as referents per scene grow from two to four, grounded recall (navy) stays at the correct-region oracle (light blue, coincident) while whole-scene recall stays at the floor. \textbf{Right:} grounding accuracy is $1.00$ at every clutter level and identical for Qwen2.5-VL-7B and -32B, so the decomposition does not require a large grounder.}
  \label{fig:density}
\end{figure}
