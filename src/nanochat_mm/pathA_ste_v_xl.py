"""STE codebook + K=64 + generic-NTP on the XL face data.

Audio STE+K=64 hit code-match 1.00 at N=5. Vision V-XC-ID at K=64 with
naive k-means already reaches code-match 0.92 at N=5. STE codebook
...
def main():
    print("=" * 70)
    print("Path A + STE codebook + K=64 on V-XC-ID-XL (423 LFW IDs)")
    print("=" * 70)
    ...
    K = 64
    # Need a dummy audio set for the constructor — use any small audio embedding
    ...
    print("\n[pretrain STE+K=64 on vision XL] ...")
    ...
    print("\n[eval] held-out V-XC-ID retrieval")
    ...
    out = Path("/home/ubuntu/multimodal-user-memory/results/pathA_V-XC-ID-xl-ste.json")
    with open(out, "w") as f:
        json.dump({"K": K, "results": results}, f, indent=2, default=str)
    print(f"\n[done] {out}")

    # Compare to non-STE V-XC-ID-xl
    naive = json.load(open("/home/ubuntu/multimodal-user-memory/results/pathA_V-XC-ID-xl.json"))
    print("\n" + "=" * 80)
    print("V-XC-ID-XL: K=64 generic-NTP vs K=64 +STE")
    print("=" * 80)
    print(f"{'N':>4} | {'RAG':>5} | {'naive retr@1':>13} | {'+STE retr@1':>12} | "
          f"{'naive cm':>9} | {'+STE cm':>8}")
    print("-" * 80)
    for N in Ns:
